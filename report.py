...
async def do_login(page):
    ...
    for frame in page.frames:
        try:
            id_input = await frame.query_selector("input[placeholder='아이디']")
            if not id_input:
                continue
...
async def navigate_to_prod(page):
    for frame in page.frames:
        try:
            ...
            continue
    raise Exception("Menu nav failed")
    ...
    for _ in range(15):
        for frame in page.frames:
            if "prod011" in frame.url:
                return frame
        await page.wait_for_timeout(1000)
    raise Exception("prod011 not found")
...
async def fetch(prod_frame, date_from, date_to):
    ...
    await prod_frame.wait_for_timeout(5000)
